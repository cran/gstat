...
data(vv)
@
Plotting this object can be done in two ways, both shown in figure
\ref{fig:map}:
<<eval=FALSE>>=
...
@
\end{center}
\caption{ sample variogram map (right) and fitted metric model (left). }
\label{fig:mm}
\end{figure}
...
@
\end{center}
\caption{ sample variogram map (right) and fitted seperable model (left). }
\label{fig:sm}
\end{figure}
...
@
which is shown in figure \ref{fig:wire}.
\begin{figure}[hbt]
\begin{center}
<<fig=TRUE,echo=FALSE>>=
library(lattice)
