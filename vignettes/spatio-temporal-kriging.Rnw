...
\href{mailto:gerard.heuvelink@wur.nl}{Gerard Heuvelink}
}
\date{\footnotesize Jun 03, 2015 }
...
% \VignetteIndexEntry{Spatio-Temporal Interpolation using gstat}
...
\begin{enumerate}[a)]
\item The {separable covariance model} assumes that the spatio-temporal covariance function can be represented as the product of a spatial and temporal term:
...
\item 
The above model extends to the \emph{product-sum covariance model} that we give here in a slightly different notation as \citet{DeCesare2001} and \citet{DeIaco2001} by 
...
\item 
Assuming identical spatial and temporal covariance functions except for spatio-temporal anisotropy, allows to use a spatio-temporal \emph{metric covariance model} where, after matching space and time by an anisotropy correction $\kappa$ (\code{stAni}), the spatial, temporal and spatio-temporal distances are treated equally resulting in a single covariance model $C_{\rm joint}$: 
...
where $\gamma_{\rm joint}$ (\code{jointVgm}) is any known variogram possibly including some nugget effect. The following line generates the model in \pkg{gstat}:
...
\item
A combination of spatial, temporal and a metric model including an anisotropy parameter $\kappa$ is found in \citet{Bilonick1988} and revisited by \citet{Snepvangers2003} as the \emph{sum-metric covariance model}: 
...
{\footnotesize
\begin{verbatim}
vgmST("sumMetric", space = spatialVgm, time = temporalVgm, 
      joint = jointVgm, stAni = stAni)
\end{verbatim}}
...
The \emph{simple sum-metric covariance model} can be obtained by:
...
\begin{tabular}{l|rlrlrr}
  ...
  & partial sill & model & \multicolumn{2}{c}{range}  & nugget & sp.-temp. sill \\ \hline  
space & 0.86 & Exp & 558 & \hspace{-2\tabcolsep}~km & 0.14 & \multirow{2}{*}{124} \\
time & 1.00 & Sph & 5.6 & \hspace{-2\tabcolsep}~days & 0.00 & \\ 
\end{tabular}
...
fit.StVariogram(empVgm, separableModel, fit.method = 7, 
                ...
                lower = c(10, 0, 0.1, 0, 0),
                upper = c(2000, 1, 12, 1, 200))
\end{verbatim}}

\item 
product-sum model (weighted MSE: 6.88)

\nopagebreak
\begin{tabular}{l|rlrlrc}

  & partial sill & model & \multicolumn{2}{c}{range}  & nugget & k\\ \hline  
space & 5.7 & Exp & 531& \hspace{-2\tabcolsep}~km  & 1.0 & \multirow{2}{*}{1.94} \\
time & 8.7 & Sph & 5.8 & \hspace{-2\tabcolsep}~days & 0.0 & \\ 
\end{tabular}
...
\begin{tabular}{l|rlrlrrl}
  ...
  & partial sill & model & \multicolumn{2}{c}{range}  & nugget & \multicolumn{2}{c}{anisotropy}\\ \hline  
joint & 123 & ${\rm Mat}_{\kappa=0.6}$ & 452 & \hspace{-2\tabcolsep}~km & 17 & 189 & \hspace{-2\tabcolsep}~km/day \\ 
\end{tabular}
...
joint    & 91.6 & Sph & 999 & \hspace{-2\tabcolsep}~km  & 7.3 & 185 & \hspace{-2\tabcolsep}~km/day \\ 
\end{tabular}
...
sumMetricModel <- vgmST("sumMetric",
                        space = vgm(20, "Sph", 150, 2.5),
                        time = vgm(10, "Exp", 2, 2.5),
                        joint = vgm(80, "Sph", 400, 2.5),
                        stAni = 150)
fit.StVariogram(empVgm, sumMetricModel, fit.method = 7,
                stAni = 118, method = "L-BFGS-B",
                control = list(parscale = c(1, 10, 0.1, 1, 0.1, 
                                            1, 1, 100, 1, 10)),
...
\begin{tabular}{l|rlrlrlc}
          ...
          & partial sill & model & \multicolumn{2}{c}{range} & \multicolumn{2}{c}{anisotropy} & sp.-temp. nugget \\ \hline  
space  & 16.4 & Sph & 67   & \hspace{-2\tabcolsep}~km     & & & \multirow{3}{*}{$\Bigg\}$ 7.3} \\
time    & 9.3 & Exp & 0.9 & \hspace{-2\tabcolsep}~days & & & \\
joint    & 91.5 & Sph & 999 & \hspace{-2\tabcolsep}~km      & 185 & \hspace{-2\tabcolsep}~km/day &  \\ 
\end{tabular}
...
simpleSumMetricModel <- vgmST("simpleSumMetric",
                              ...
                              nugget = 10, stAni = 150)
fit.StVariogram(empVgm, simpleSumMetricModel,  
...
model & joint & Exp+Exp & Exp+Sph & Sph+Exp & Sph+Sph & ${\rm Mat}_{\kappa=0.6}$ \\ \hline  
{separable} & $~~\cdot$ & 9.87 & \bf{6.82} & 10.42 & 7.50 & $\cdot~~$ \\
{product-sum} & $~~\cdot$ & 10.09 & \bf{6.88} & 10.64 & 7.59 & $\cdot~~$ \\
{metric} & $~~\cdot$ & 10.25 & $\cdot~~$ & $\cdot~~$ & 10.59 & \bf{10.05} \\
\multirow{2}{*}{sum-metric} & Exp &  4.10 & 3.60 & 3.89 & 3.32 & $\cdot~~$ \\
& Sph & 3.74 & 3.73 & \bf{3.31} & 3.32 & $\cdot~~$ \\
\multirow{2}{*}{simple sum-metric}& Exp &  4.10 & 3.60 & 3.94 & 3.32 & $\cdot~~$ \\
& Sph & 3.74 & 3.98 & \bf{3.31} & 3.36 & $\cdot~~$ \\ 
\end{tabular}
\end{table}

The variogram parameters are numerically optimised using the function \code{fit.StVariogram} and the \code{L-BFGS-B} routine of \code{optim}. The parameter \code{fit.method} that controls the weighing of the residuals between empirical and model surface of \code{fit.StVariogram} is set to \code{7}. A full list of all weighting schemes is presented in Table~\ref{tab:weighting}. In our application, the residuals are multiplied by the number of pairs in the corresponding spatio-temporal bin divided by the metric distance: $N_j/(h_j^2 + {\rm stAni}^2\cdot u_j^2)$. The spatio-temporal anisotropy is estimated beforehand and fixed at 118~km/day. This weighting scheme puts higher confidence in lags filled with many pairs of spatio-temporal locations, but respects to some degree the need of an accurate model for short distances, as these short distances are the main source of information in the prediction step.

For comparison with classical approaches, we interpolate across Germany iteratively for each single day using all available data for variogram estimation. The purely spatial empirical variogram can directly be obtained from the empirical spatio-temporal variogram, by fixing the temporal lag at 0 separation. From the same set of variogram models as investigated for the spatio-temporal models, the exponential model (partial sill:~68, range:~234~km, nugget:~14) is the best suited based on the optimisation criterion. Alternatively, we could have fitted the spatial variogram for each day separately using observations from that day only. However, given the small number of observation stations, this produced unstable variograms for several days and we decided to use the single spatial variogram derived from all spatio-temporal locations treating time slices as uncorrelated copies of the spatial random field.

Once the best fitting spatio-temporal variogram model is identified, the interpolation can be executed with the help of the function \code{krigeSt}. We use the sum-metric model that obtained the smallest RMSE (compare Table~\ref{tab:vgmFits}) to produce a gridded prediction. The interpolation domain consists of daily values for a regular grid spanning over Germany in UTM projection. The cell size is $10~\rm{km}\times10~\rm{km}$. Figure~\ref{fig:pred_daily} shows the interpolated grid for the same days as Figure~\ref{fig:dailyMeans} alongside with all sampling locations. Figure~\ref{fig:timeseries} shows the observed and predicted times series at a single location along with its 95~\% prediction intervals. An animation of the entire year of daily mean ${\rm PM}_{10}$ prediction maps can be viewed online.\footnote{\url{http://ifgi.uni-muenster.de/~b_grae02/publications/animate_PM10.pdf}}.

The interpolated maps are generated by

{\footnotesize
\begin{verbatim}
kr = krigeST(PM10~1, data = DE_RB_2005, 
...
\begin{table}
\caption{Leave one out cross-validation results}\label{tab:cv}
\centering
\begin{tabular}{lrr|rrrr}

covariance model & wMSE & neigh. &  RMSE   &   MAE   &    ME   &   COR \\ \hline
pure Spatial & & 10 & 6.14 & 4.08 & -0.01 & 0.84 \\ 
separable &\hspace{-\tabcolsep}[6.46]&     10 & 6.09 & 4.05 & -0.01 & 0.84  \\ 
product-sum &\hspace{-\tabcolsep}[5.96]& 10 &  6.46 & 4.29 & -0.03 & 0.82  \\ 
metric &\hspace{-\tabcolsep}[9.76]&          10 &  6.09 & 4.05 & 0.01 & 0.84  \\ 
sum-metric &\hspace{-\tabcolsep}[3.07]&   10 &  6.16 & 4.08 & -0.05 & 0.84  \\ 
simple sum-metric &\hspace{-\tabcolsep}[3.07]& 10 & 6.14 & 4.08 & -0.03 & 0.84  \\  \hline
pure Spatial & & 50 & 6.10 & 4.07 & 0.00 & 0.84  \\
separable &\hspace{-\tabcolsep}[6.46]& 50     &  6.06 & 4.04 & 0 & 0.84  \\ 
product-sum &\hspace{-\tabcolsep}[5.96]& 50 &  6.42 & 4.28 & -0.05 & 0.83  \\ 
metric &\hspace{-\tabcolsep}[9.76]& 50          &  6.05 & 4.04 & 0.01 & 0.85  \\ 
sum-metric &\hspace{-\tabcolsep}[3.07]& 50   &  6.14 & 4.08 & -0.03 & 0.84  \\ 
simple sum-metric &\hspace{-\tabcolsep}[3.07]& 50 &  6.14 & 4.08 & -0.03 & 0.84 \\ 
\end{tabular}
\end{table}
...
Besides some publications where the authors of this paper were involved in, such as \cite{Kilibarda2014}, the software presented here has proven useful in several independent publications, verbatims of which are \citep{marek, biondi, hu, yoon}.

\section*{Acknowledgements}
This research has partly been funded by the German Research Foundation (DFG) under project number PE 1632/4-1.
